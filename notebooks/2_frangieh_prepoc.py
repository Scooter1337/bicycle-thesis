...
# de-normalize and round up (MEMORY OPTIMIZED: stays sparse)
norm_factor = data.obs["UMI_count"].values / 1.0e6
# Apply expm1 + scaling + rounding only to nonzero entries
data_sp.data = np.expm1(data_sp.data)
# Scale by norm_factor per cell
data_sp.data = data_sp.data * np.repeat(norm_factor, np.diff(data_sp.indptr))
# Check and round
print(np.greater_equal(np.abs(data_sp.data - np.rint(data_sp.data)), 0.01).any())
data_sp.data = np.rint(data_sp.data)
data.X = data_sp  # Keep sparse
...
def generate_nodags_data(adata_path):
    ...
    # MEMORY OPTIMIZED: convert to float32 and process efficiently
    data = sp.csr_matrix.toarray(adata.X).astype(np.float32)
...
# Free memory from adata_gf and adata_small
del adata_gf, adata_small
gc.collect()
...
# %%
# MEMORY OPTIMIZED: Process each condition one at a time
...
for name, train_samples_, valid_samples_, train_interv_, valid_interv_ in conditions_to_process:
    ...
    del adata, data, obs_data, obs_data_mu, dataset_train, dataset_train_targets
    del datset_test, dataset_test_targets, samples, sim_regimes
