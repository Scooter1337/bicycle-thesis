...
OUT_PATH = Path("/Users/luca/Developer/Universiteit/leiden-university/bachelor-project/imae")
...
user_dir = "/Users/luca/Developer/Universiteit/leiden-university/bachelor-project"
MODEL_PATH = Path(os.path.join(user_dir, "reproduce3/new_results"))
...
# load in wildtype.npy from nodags_data/{DATASET}/wildtype.npy
if WILDTYPE:
    wildtype = np.load(DATA_PATH / f"{DATASET}/wildtype.npy", allow_pickle=True)
    print('Loaded wildtype:',wildtype)
    assert wildtype.shape[0] == 61, 'Wildtype shape mismatch'

# Load Count Data for Training
DATA_PATH = Path("/Users/luca/Developer/Universiteit/leiden-university/bachelor-project/nodags_data/COUNTS")
train_loader = torch.load(DATA_PATH / f"{DATASET}/training_data/train_loader.pth", weights_only=False)
validation_loader = torch.load(DATA_PATH / f"{DATASET}/validation_data/validation_loader.pth", weights_only=False)
test_loader = torch.load(DATA_PATH / f"{DATASET}/test_data/test_loader.pth", weights_only=False)
labels = np.load(DATA_PATH / f"{DATASET}/labels.npy", allow_pickle=True)

# Load Original, Normalized Frangieh Data
ORIGINAL_DATA_PATH = Path("/Users/luca/Developer/Universiteit/leiden-university/bachelor-project/nodags_data")
test_loader_normalized = torch.load(ORIGINAL_DATA_PATH / f"{DATASET}/test_data/test_loader.pth", weights_only=False)
...
# Frangieh Data
FRANGIEH_DATA_PATH = Path("/Users/luca/Developer/Universiteit/leiden-university/bachelor-project/SCP1064")
...
filename = MODEL_PATH / "nodags_split_False_adam_1024_True_61pretrain5000_0.1_1.0_1_0.1_1_0.001_0.1_1.0_1_0.1_0.001_0_False/last.ckpt"
...
for gene in tqdm(range(gene_start, gene_end)): #tqdm(range(gt_interv.shape[0])):
    ...
    for gene in range(61):
        if WILDTYPE:
            # if wildtype, take average of wildtype for that gene (average of column)
            average = wildtype[gene].mean()
            prior_pred_mat[:,gene] = average
        else:
            prior_pred_mat[:,gene] = model.predict_percentages(test_loader.dataset[:])[:,gene]
        # FOR WILDTYPE LOAD AVERAGE OF COLUMN HERE ^
    prior_pred_mat = torch.log(prior_pred_mat*marginal_counts + 1)
    imae = torch.abs( (prior_pred_mat - data_mat_normalized)*pred_unintervened_mat ).sum() / pred_unintervened_mat.sum()
