...
if __name__ == "__main__":
    ...
    train_loader = torch.load(DATA_PATH / f"nodags_data/{DATASET}/training_data/train_loader.pth", weights_only=False)
    validation_loader = torch.load(DATA_PATH / f"nodags_data/{DATASET}/validation_data/validation_loader.pth", weights_only=False)
    test_loader = torch.load(DATA_PATH / f"nodags_data/{DATASET}/test_data/test_loader.pth", weights_only=False)
    labels = np.load(DATA_PATH / f"nodags_data/{DATASET}/labels.npy", allow_pickle=True)

    samples, regimes, idx, splits = next(iter(train_loader))
    ...
    n_contexts = n_genes
    rank_w_cov_factor = n_genes  # Full rank for numerical stability # n_genes - 1  # Same as dictys: #min(TFs, N_GENES-1)
    n_contexts = n_genes # Not using unperturbed cells, to be comparable to NODAGS-Flow
    ...
    swa = 0
    x_distribution = "Multinomial"  # "Poisson" if "random" in graph else
    intervention_type = "Cas9"
    # DATA
    validation_size = 0.0
    # MODEL
    ...
    use_latents = True
    # RESULTS
    name_prefix = f"nodags_split_{use_encoder}_{optimizer}_{batch_size}_{lyapunov_penalty}_{rank_w_cov_factor}"
    SAVE_PLOT = True
    CHECKPOINTING = True
    ...
    for scale_kl in [1.0]:
        for scale_l1 in [0.1]: # 0.1
            for scale_spectral in [0, 1]:
                for scale_lyapunov in [0.1, 1, 10]:
                    file_dir = (
                        name_prefix
                        + f"pretrain{n_epochs_pretrain_latents}_{scale_l1}_{scale_kl}_{scale_spectral}_{scale_lyapunov}_{SEED}_{lr}_{scale_l1}_{scale_kl}_{scale_spectral}_{scale_lyapunov}_{gradient_clip_val}_{swa}_{USE_INITS}"
                    )
                    print(file_dir)
                    ...
                    if use_latents and n_epochs_pretrain_latents > 0:
                        ...
                        pretrain_callbacks = [
                            RichProgressBar(refresh_rate=1),
                            GenerateCallback(
                                str(Path(final_plot_name).with_suffix("")) + '_pretrain', plot_epoch_callback=plot_epoch_callback,
                            ),
                        ]
